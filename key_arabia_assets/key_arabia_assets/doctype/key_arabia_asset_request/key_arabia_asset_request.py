...
from key_arabia_assets.utils import append_audit_comment, validate_company_access


class KeyArabiaAssetRequest(Document):
    ...
    def approve_and_create_asset(self):
        ...
        asset.key_arabia_asset_head = self.asset_head
        asset.key_arabia_asset_category = self.asset_category
        asset.key_arabia_plate_number = self.plate_number
        asset.key_arabia_chassis_number = self.chassis_number
        asset.key_arabia_engine_number = self.engine_number
        asset.key_arabia_registration_expiry = self.registration_expiry
        asset.key_arabia_insurance_expiry = self.insurance_expiry
        asset.key_arabia_salik_tag = self.salik_tag
        asset.key_arabia_custody_status = "In Company Stock"
        asset.key_arabia_available_for_issue = 1
        asset.insert()
