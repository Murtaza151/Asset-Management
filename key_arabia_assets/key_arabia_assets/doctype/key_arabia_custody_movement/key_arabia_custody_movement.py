...
class KeyArabiaCustodyMovement(Document):
    ...
    def _validate_links(self):
        ...
        for rider_field in ("from_rider", "to_rider"):
            ...
            if rider and frappe.db.get_value("Key Arabia Rider", rider, "company") != self.company:
    ...
    def _validate_reversal(self):
        ...
        if self.source_movement:
            source = frappe.get_doc("Key Arabia Custody Movement", self.source_movement)
    ...
    def _lock_and_validate_latest_state(self):
        ...
        latest = frappe.db.sql(
            ...
            from `tabKey Arabia Custody Movement`
        ...
        if self.to_rider:
            conflicting = frappe.db.sql(
                ...
                where a.name != %s and a.company = %s
                  and a.key_arabia_current_rider = %s
                  and a.key_arabia_custody_status = 'Assigned to Rider'
                limit 1
                """,
    ...
    def _apply_asset_snapshot(self):
        ...
        values = {
            "key_arabia_custody_status": status,
            "key_arabia_current_rider": self.to_rider if self.to_holder_type == "Rider" else None,
            "key_arabia_available_for_issue": 1 if self.to_holder_type == "Company" and self.condition not in {"Damaged", "Under Repair", "Lost", "Scrapped"} else 0,
            "key_arabia_blocking_reason": self.blocking_reason,
        }
        if self.to_location:
            values["location"] = self.to_location
        if self.to_holder_type == "Rider":
            values["key_arabia_last_handover"] = self.occurred_at
        if self.to_holder_type == "Company":
            values["key_arabia_last_return"] = self.occurred_at
    ...
    def _rebuild_asset_snapshot(self):
        ...
        if latest_name:
            frappe.get_doc("Key Arabia Custody Movement", latest_name)._apply_asset_snapshot()
        else:
            frappe.db.set_value(
                "Asset",
                self.asset,
                {"key_arabia_custody_status": "In Company Stock", "key_arabia_current_rider": None, "key_arabia_available_for_issue": 1},
            )
