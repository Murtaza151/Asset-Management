...
from key_arabia_assets.utils import validate_company_access, validate_time_range


class KeyArabiaReplacementAssignment(Document):
    def validate(self):
        ...
        if frappe.db.get_value("Key Arabia Rider", self.rider, "company") != self.company:
        ...
        if self.status == "Active":
            conflict = frappe.db.exists("Key Arabia Replacement Assignment", {"name": ("!=", self.name), "replacement_asset": self.replacement_asset, "status": "Active"})
            if conflict:
