...
def execute(filters=None):
    ...
    if not filters.source_type or filters.source_type == "Fine":
        data += frappe.db.sql("""select 'Fine' source_type,'DexaFleet Fine' doctype,name reference,issued_at transaction_at,plate_number plate,amount,match_status,match_reason reason from `tabDexaFleet Fine` where company=%(company)s and status='Exception' and date(issued_at) between %(from_date)s and %(to_date)s""", filters, as_dict=True)
    if not filters.source_type or filters.source_type == "Salik":
        data += frappe.db.sql("""select 'Salik' source_type,'DexaFleet Salik Transaction' doctype,name reference,trip_at transaction_at,plate_number plate,amount,match_status,match_reason reason from `tabDexaFleet Salik Transaction` where company=%(company)s and status='Exception' and date(trip_at) between %(from_date)s and %(to_date)s""", filters, as_dict=True)
