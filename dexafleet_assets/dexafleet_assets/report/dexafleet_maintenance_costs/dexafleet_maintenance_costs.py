...
def execute(filters=None):
    ...
    columns = [
        {"fieldname":"job","label":_("Job"),"fieldtype":"Link","options":"DexaFleet Maintenance Job","width":150},
    ...
    data = frappe.db.sql(f"""select name job,invoice_number invoice,job_date date,asset,workshop,items_total,labour,grand_total,rider_charge from `tabDexaFleet Maintenance Job` where {' and '.join(conditions)} order by job_date desc""", filters, as_dict=True)
    return columns, data
