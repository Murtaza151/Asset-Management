...
class DexaFleetRiderChargeRun(Document):
    def validate(self):
    ...
    def calculate(self):
        ...
        self.sources = []
        self._append_sources("DexaFleet Fine", "issued_at")
        self._append_sources("DexaFleet Salik Transaction", "trip_at")
    ...
    def _append_maintenance_sources(self):
        ...
        existing = frappe.get_all("DexaFleet Charge Source", filters={"source_type": "DexaFleet Maintenance Job", "source_name": ["in", [row.name for row in rows]]}, pluck="source_name") if rows else []
        for row in rows:
            if row.current_rider and row.name not in existing:
                self.append("sources", {"source_type": "DexaFleet Maintenance Job", "source_name": row.name, "rider": row.current_rider, "asset": row.asset, "transaction_at": row.job_date, "amount": row.rider_charge, "status": "Included"})

    @frappe.whitelist()
    def approve(self):
        if self.status not in {"Calculated", "Reviewed"}:
            frappe.throw(_("Run must be Calculated or Reviewed before approval."))
        if "DexaFleet Finance Manager" not in frappe.get_roles() and "System Manager" not in frappe.get_roles():
            frappe.throw(_("DexaFleet Finance Manager role is required."))
