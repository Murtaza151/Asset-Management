# Key Arabia Assets — Administrator Guide

## Entry points

- Search for **Key Arabia Assets** to open the operational dashboard.
- Standard list/form/report views remain available for every DocType.
- Asset records receive a **Key Arabia Operations** section after installation.

## Roles

- **Key Arabia Asset User** — operational request entry and read access.
- **Key Arabia Asset Manager** — riders, assets, custody submission, and asset approvals.
- **Key Arabia Operations Manager** — custody, allocation, packages, and replacements.
- **Key Arabia Workshop User** — draft/in-progress maintenance job entry.
- **Key Arabia Workshop Manager** — maintenance submission and catalogue requests.
- **Key Arabia Finance User** — Fine/Salik imports, matching, and charge calculations.
- **Key Arabia Finance Manager** — charge-run approval and finance reconciliation.
- **Key Arabia Company Admin** — company configuration and protected approvals.
...
1. Create a **Key Arabia Asset Request**.
...
5. Use **Key Arabia Custody Movement** for every subsequent handover, return, workshop, police, client, correction, or reversal event.
...
Maintenance prices are read from **Key Arabia Maintenance Item** and copied into each job row. This historical price snapshot is not changed by later catalogue updates.
...
Use **Key Arabia Import Batch** for CSV validation and commit. Warnings require explicit confirmation; blocked rows cannot commit.
